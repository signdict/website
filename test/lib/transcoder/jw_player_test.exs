defmodule SignDict.Transcoder.JwPlayerTest do
  ...
  describe "upload_video/2" do
    ...
    defmodule MockHTTPoisonUpload do
      def get!(url) do
        if String.starts_with?(url, "http://api.jwplatform.com/v1/videos/create") do
          %{body: "{\"status\": \"ok\", \"rate_limit\": {\"reset\": 1494102000, \"limit\": 60, \"remaining\": 59}, \"video\": {\"key\": \"m7xjXN0r\"}}"}
        end
      end
    end
    ...
    defmodule MockHTTPoisonUploadFailed do
      def get!(url) do
        if String.starts_with?(url, "http://api.jwplatform.com/v1/videos/create") do
          %{body: "{\"status\": \"error\", \"rate_limit\": {\"reset\": 1494102000, \"limit\": 60, \"remaining\": 59}}"}
        end
      end
    end
    ...
    test "it uploads a video" do
      ...
      {state, _result} = SignDict.Transcoder.JwPlayer.upload_video(video, MockHTTPoisonUpload, ExqMock)
      ...
      assert video.metadata["jw_status"] == %{
        "rate_limit" => %{
          "limit" => 60, "remaining" => 59, "reset" => 1494102000
        }, "status" => "ok", "video" => %{"key" => "m7xjXN0r"}
      }
    ...
    test "if the upload fails, the video is updated accordingly" do
      video            = insert(:video_with_entry, %{metadata: %{"source_mp4" => "Zug2.mp4"}})
      {state, _result} = SignDict.Transcoder.JwPlayer.upload_video(video, MockHTTPoisonUploadFailed)
      video            = Repo.get(Video, video.id)
      video_id         = video.id
      assert state == :error
  ...
  describe "check_status/2" do
    ...
    test "it returns :done and updates the video url and thumbnail if the video is done" do
      ...
      assert video.metadata == %{
        "jw_status" => %{"description" => "",
                "feed_instance_id" => "cf789865-e28f-4d3c-b390-b7c44dabd19d",
                "kind" => "Single Item",
                "playlist" => [%{"description" => "", "duration" => 6,
                   "image" => "https://cdn.jwplayer.com/thumbs/4OCxwoAI-720.jpg",
                   "link" => "https://cdn.jwplayer.com/previews/4OCxwoAI",
                   "mediaid" => "4OCxwoAI", "pubdate" => 1493273662,
                   "sources" => [%{"file" => "https://cdn.jwplayer.com/manifests/4OCxwoAI.m3u8",
                      "height" => 360,
                      "type" => "application/vnd.apple.mpegurl",
                      "width" => 480},
                    %{"file" => "https://cdn.jwplayer.com/videos/4OCxwoAI-xgdDoosc.mp4",
                      "height" => 240, "label" => "180p", "type" => "video/mp4",
                      "width" => 320},
                    %{"file" => "https://cdn.jwplayer.com/videos/4OCxwoAI-cqrEIsCv.mp4",
                      "height" => 360, "label" => "270p", "type" => "video/mp4",
                      "width" => 480}], "title" => "Zug",
                   "tracks" => [%{"file" => "https://cdn.jwplayer.com/strips/4OCxwoAI-120.vtt",
                      "kind" => "thumbnails"}]}], "title" => "Zug"},
              "jw_video_id" => "4OCxwoAI"}
    end

    test "it returns :transcoding when the video is still in progress" do
      video  = insert(:video_with_entry, %{video_url: nil, thumbnail_url: nil, metadata: %{"jw_video_id" => "4OCxwoAI"}})
      ...
      assert video.metadata == %{
        "jw_status" => %{
          "rate_limit" => %{
            "limit" => 60, "remaining" => 59, "reset" => 1494093660
          }, "status" => "ok",
        "video" => %{
          "author" => nil, "custom" => %{},
          "date" => 1493273662, "description" => nil,
          "duration" => "6.84", "error" => nil,
          "expires_date" => nil, "key" => "4OCxwoAI",
          "link" => nil, "md5" => "fab44d8176b00d4d38b902281d3f2a0b",
          "mediatype" => "video", "size" => "1135854",
          "sourceformat" => nil, "sourcetype" => "file",
          "sourceurl" => nil, "status" => "processing",
          "tags" => nil, "title" => "Zug", "trim_in_point" => nil,
          "trim_out_point" => nil, "updated" => 1493273792,
          "upload_session_id" => nil, "views" => 7}
        },
        "jw_video_id" => "4OCxwoAI"}
    end
