defmodule SignDict.Services.EntryVideoLoaderTest do
  ...
  setup do
    ...
    video_1_with_votes = Repo.get(Video, video_1.id) |> Repo.preload(:user) |> Video.with_vote_count
    video_2_with_votes = Repo.get(Video, video_2.id) |> Repo.preload(:user) |> Video.with_vote_count

    {
      :ok,
      entry: entry, user_1: user_1, user_2: user_2, user_3: user_3,
      video_1: video_1_with_votes, video_2: video_2_with_votes,
    }
  end

  test "it returns the highest voted video for an entry",
       %{entry: entry, video_1: video_1, video_2: video_2} do
  ...
  test "it returns the voted video if the user has voted one for the entry",
        %{entry: entry, video_1: video_1, video_2: video_2, user_3: user} do
