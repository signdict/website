defmodule SignDict.EmbedControllerTest do
  ...
  describe "show/2 entry attributes" do
    ...
    test "it shows an error if the entry does not have any videos and no video id is given", %{conn: conn} do
      entry = insert(:entry)
      conn = get conn, embed_path(conn, :show, entry)
      assert html_response(conn, 200) =~ "Sorry, no sign was found"
    end

    test "it shows an error if the entry does not have any videos and a video id is given", %{conn: conn} do
      entry = insert(:entry)
