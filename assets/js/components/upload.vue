      ...
      <header>
        <div class="o-grid o-grid--wrap">
          <div class="o-grid__cell o-grid__cell--width-100 o-grid__cell--width-50@medium">
            <a href="/">
              <img src="/images/logo.svg" :alt="$t('SignDict Logo')" class="so-navigation__logo">
            </a>
    ...
    <div class="sc-main o-container o-container--medium upload--content">
      <div class="o-grid__cell o-grid__cell--width-100">
        ...
        <div v-if="currentPanel != 'thankyou'">
          <router-link to="/cutter" class="upload--back">&lt;&lt; {{ $t('Back') }}</router-link>
        </div>
        <div class="upload--loading" v-if="currentPanel == 'loading'">{{ $t('Please wait...') }}</div>
        <div class="upload--transfer" v-else-if="currentPanel == 'uploading'">
          <div class="upload--transfer--progressbar">
            ...
            <div class="upload--transfer--progressbar--text">{{ $t('Uploading') }}</div>
          </div>
        </div>
        <div class="upload--thankyou" v-else-if="currentPanel == 'thankyou'">
          <p
            class="upload--thankyou--text"
          >{{ $t('Thank you for uploading your video. It will be published after a review within the next 48 hours.') }}</p>
        ...
        <div class="upload--error" v-else-if="currentPanel == 'error'">
          <p
            class="sc-alert sc-alert--error"
            role="alert"
          >{{ $t('Sorry, there was a problem during the upload. Please contact us if this error appears more than once.') }}</p>
        </div>
        <div class="upload--submit" v-else-if="currentUser">
          <p
            class="upload--submit--legal"
            v-locale="{ locale: $root.locale, tra: 'By pressing the upload button you will publish the video using the |CC-BY-SA 4.0| license.'}"
          >
            <span></span>
        ...
        <div class="upload--login-or-register" v-else>
          ...
          <div>
            <button
              class="sc-button"
              v-on:click.stop="showLoginPanel"
            >{{ $t('I already have a SignDict account') }}</button>
          </div>
            ...
            <div class="upload--login" v-if="currentPanel == 'login'">
              <div v-if="loginError">
                <p
                  class="sc-alert sc-alert--error"
                  role="alert"
                >{{ $t('The email or password was wrong.') }}</p>
              </div>
          ...
          <div>
            <button
              class="sc-button"
              v-on:click.stop="showRegisterPanel"
            >{{ $t('I don\'t have a SignDict account') }}</button>
          </div>
            ...
            <div class="upload--register" v-if="currentPanel == 'register'">
              <div v-if="registerError">
                <p
                  class="sc-alert sc-alert--error"
                  role="alert"
                >{{ $t('Oops, something went wrong! Please check the errors below.') }}</p>
              </div>
              <form
                accept-charset="UTF-8"
                action="/users"
                class="user-form"
                v-on:submit.prevent="submitRegisterUser"
              >
                <div>
                  ...
                  >
                  <span
                    class="sc-hint sc-hint--error"
                    v-if="registerError.name"
                  >{{ registerError.name }}</span>
                </div>
                ...
                <div>
                  ...
                  <span
                    class="sc-hint sc-hint--error"
                    v-if="registerError.password_confirmation"
                  >{{ registerError.password_confirmation }}</span>
                </div>
                ...
                <div>
                  <label class="sc-field sc-field--choice">
                    ...
                    <input
                      id="user_want_newsletter"
                      name="user[want_newsletter]"
                      type="checkbox"
                      value="true"
                      v-model="registerWantNewsletter"
                    >
...
function refreshUser(self) {
  self.$http.get("/api/current_user").then(
    response => {
...
export default {
  ...
  methods: {
    ...
    uploadVideo: function() {
      ...
      formData.append(
        "entry_id",
        document.getElementById("app").getAttribute("data-entry-id")
      );
