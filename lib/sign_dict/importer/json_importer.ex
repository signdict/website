defmodule SignDict.Importer.JsonImporter do
  ...
  defp import_video(filename) do
    file = "#{Ecto.UUID.generate}-#{Path.basename(filename)}"
  ...
  def query_for_entry(language, text, description) do
    from(entry in Entry, where: entry.language_id == ^language.id and entry.text == ^text and entry.description == ^description)
  end
  def query_for_entry(language, text) do
    from(entry in Entry, where: entry.language_id == ^language.id and entry.text == ^text and (entry.description == "" or is_nil(entry.description)))
  end
