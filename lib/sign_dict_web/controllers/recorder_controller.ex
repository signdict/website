defmodule SignDictWeb.RecorderController do
  ...
  def index(conn, %{"entry_id" => entry_id}) do
    ...
    if is_nil(entry) do
      ...
      render(
        conn, "index_#{Gettext.get_locale(SignDictWeb.Gettext)}.html", layout: {SignDictWeb.LayoutView, "app.html"},
        entry: entry
      )
    end
  end
  ...
  def new(conn, %{"entry_id" => entry_id}) do
    if is_nil(Repo.get(Entry, entry_id)) do
      ...
      render(
        conn, "new.html", layout: {SignDictWeb.LayoutView, "recorder.html"},
        entry_id: entry_id
      )
    end
  end
